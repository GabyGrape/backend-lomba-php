...
.page-title {
    ...
    color:#555; padding:10px 16px 6px;
    background:#f5f5f5;
}
...
.btn-back svg { width:22px; height:22px; transition:0.3s; }
...
.product-name {
    color:#ffb800; font-size:44px;
    font-family:'Montserrat',sans-serif;
    margin-bottom:10px; line-height:1.1;
}

.product-desc {
    color:#7286D3; font-size:16px; margin-bottom:25px;
}

.price-tag {
    color:#7286D3; font-size:26px;
...
.btn-pay {
    background:#ffb800; color:#001a57;
    padding:13px 28px; border-radius:12px;
    border:none; cursor:pointer;
    font-weight:700; font-size:14px;
    font-family:'Poppins',sans-serif; transition:0.2s;
...
<div class="footer-nav">
    <div class="footer-inner">
        ...
        <div class="footer-btns">
             <button class="btn-cancel" onclick="batalBeli()">
                Batal Beli
            </button>
...
if(selectedProduct){
    ...
    if(!existing){
        ...
        // sinkronkan qty box dengan qty cart
        qty = existing.qty;
    }
...
if(selectedProduct){
    ...
    document.getElementById("productImg").src =
        selectedProduct.gambar_url || `https://picsum.photos/400/400?random=${selectedProduct.id}`;
...
function batalBeli(){
    ...
    const yakin = confirm("Yakin ingin membatalkan semua pesanan?");
...
async function checkout(){
    // Tambah item yang sedang ditampilkan ke cart dulu kalau belum
    if(cart.length === 0){
        // Auto tambah item yang sekarang
        cart.push({
            id:    selectedProduct.id,
            nama:  selectedProduct.nama_menu,
    ...
    // Gunakan waktu sekarang + 30 menit sebagai default pickup
