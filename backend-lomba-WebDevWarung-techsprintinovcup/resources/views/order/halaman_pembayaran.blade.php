...
<head>
    ...
    <style>
        *{ margin:0; padding:0; box-sizing:border-box; font-family:'Poppins',sans-serif; }
        ...
        .btn-back svg{ width:28px; height:28px; transition:0.3s; }
        ...
        .qris-card h2{
            color:#ffb800; font-size:32px;
            margin-bottom:8px;
            font-family:'Montserrat',sans-serif;
        }

        .order-info{
            font-size:13px; color:#aab8d8;
            margin-bottom:20px;
        }
        ...
        .qris-card p.note{
            color:#ffb800; font-size:13px; margin-bottom:50px;
        }
        ...
        .btn-download{
            ...
            border-radius:12px;
            display:flex; justify-content:center; align-items:center;
            transition:0.3s; color:#001a57; text-decoration:none;
        }
        .btn-download svg{ width:26px; height:26px; fill:none; }
        .btn-download:hover{ background-color:#ffb800; transform:scale(1.1); color:white; }
    </style>
</head>
<body>
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    // Ambil total dari localStorage (disimpan saat checkout)
    ...
    // Tampilkan info order jika ada
    if(lastOrder && lastOrder.id){
        document.getElementById("orderInfo").textContent =
            `Order #${lastOrder.id} • ${lastOrder.status || 'Menunggu Pembayaran'}`;
    } else {
        document.getElementById("orderInfo").textContent = "Silakan scan QRIS di bawah";
    }

    // Coba ambil QRIS dari data merchant (kalau ada di localStorage)
